...
export default function TeacherLoginModal() {
  ...
  // A logged-out staff member bounced off a protected page lands here with
  // ?next=<returnPath> (see app/components/ProtectedRoute.tsx) — open the
  // login modal automatically so they aren't stuck on the public homepage.
  useEffect(() => {
  ...
  return (
    <>
      <button
        ...
        className="shrink-0 rounded-xl border border-slate-950/10 bg-white/70 px-3 py-2 text-xs font-bold text-slate-700 shadow-sm transition hover:bg-white active:scale-[0.98]"
      >
        Staff Login
      </button>

      {open && (
        <div style={modalOverlayStyle} onClick={closeLoginModal}>
            ...
            <div style={modalHeaderStyle}>
              <div>
                ...
                <p style={{ margin: "4px 0 0", color: "#6b7280", fontSize: "13px" }}>
                  For teachers, admins and headmasters
                </p>
              </div>

              <button type="button" onClick={closeLoginModal} style={closeButtonStyle}>
                ×
              </button>
            </div>

            <form onSubmit={handleLogin} style={{ display: "grid", gap: "16px", marginTop: "16px" }}>
              <div>
              ...
              <div>
                ...
                <button type="button" onClick={openResetModal} style={forgotButtonStyle}>
                  Forgot password?
                </button>
              </div>
              ...
              {message && (
                <p style={{ margin: 0, color: "#b91c1c", fontSize: "13px", textAlign: "center" }}>
                  {message}
                </p>
              )}
            </form>
