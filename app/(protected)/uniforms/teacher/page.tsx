...
function getUniformDate(item: UniformGiven) {
...
function extractPaymentItems(payment: UniformPayment) {
  const selected = Array.isArray(payment.selected_items) ? payment.selected_items : [];

  const names = selected
    .map((item: any) => clean(item?.label || item?.item_name || item?.name))
    .filter(Boolean);
...
function buildStudentUniformRecords(
  uniformsGiven: UniformGiven[],
  payments: UniformPayment[]
): StudentUniformRecord[] {
  const map = new Map<string, StudentUniformRecord>();

  uniformsGiven.forEach((item) => {
    ...
    if (!map.has(key)) {
      map.set(key, {
        ...
        payment_status: "none",
        latest_receipt: null,
        latest_date: getUniformDate(item),
      });
    }

    const record = map.get(key)!;
    record.latest_date = newerDate(record.latest_date, getUniformDate(item));

    const uniformKey = clean(item.item_name).toLowerCase();
    const existing = record.uniforms.find((uniform) => uniform.key === uniformKey);

    if (existing) {
      existing.quantity += numberValue(item.quantity_given) || 1;
      existing.latest_date = newerDate(existing.latest_date, getUniformDate(item));
    } else {
      record.uniforms.push({
        key: uniformKey,
  ...
  payments.forEach((payment) => {
    const studentId = clean(payment.student_id);
    const className = clean(payment.class_name);
    const key = `${studentId}__${className}`;

    if (!map.has(key)) {
      map.set(key, {
        student_id: studentId,
        student_name: clean(payment.student_name),
        class_name: className,
        uniforms: [],
        total_paid: 0,
        balance: 0,
        payment_status: "none",
        latest_receipt: null,
        latest_date: payment.created_at || null,
      });
    }

    const record = map.get(key)!;
    record.latest_date = newerDate(record.latest_date, payment.created_at || null);

    extractPaymentItems(payment).forEach((itemName) => {
      const uniformKey = itemName.toLowerCase();
      const existing = record.uniforms.find((uniform) => uniform.key === uniformKey);
  ...
  map.forEach((record) => {
    ...
    record.balance = studentPayments.reduce((sum, payment) => {
      const total = numberValue(payment.total_amount);
      const paid = numberValue(payment.amount_paid);
...
export default function UniformsTeacherPage() {
  ...
  const [uniformsGiven, setUniformsGiven] = useState<UniformGiven[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadTeacherAndData() {
      try {
        ...
        await loadUniformData(classNames);
      } catch (error: any) {
        setLoadError(error?.message || "Failed to load teacher uniforms view.");
        setCheckingUser(false);
      }
    }
  ...
  async function loadUniformData(classes: string[] = assignedClassNames) {
    if (classes.length === 0) {
      setUniformsGiven([]);
      setPayments([]);
      return;
    ...
    try {
      ...
      const filteredGiven = ((givenRes.data || []) as UniformGiven[]).filter((item) =>
        allowed.includes(clean(item.class_name).toLowerCase())
      );

      const filteredPayments = ((paymentsRes.data || []) as UniformPayment[]).filter((item) =>
        allowed.includes(clean(item.class_name).toLowerCase())
      );

      setUniformsGiven(filteredGiven);
      setPayments(filteredPayments);
    } catch (error: any) {
      setLoadError(error?.message || "Failed to load uniform records.");
    } finally {
      setLoading(false);
    }
  }
  ...
  const records = useMemo(() => {
    const all = buildStudentUniformRecords(uniformsGiven, payments);
    ...
    return all.filter((record) => {
      ...
      const matchesSearch =
        ...
        record.uniforms.some((uniform) => uniform.item_name.toLowerCase().includes(q));

      return matchesClass && matchesSearch;
    });
  }, [uniformsGiven, payments, search, classFilter]);
        ...
        <header className="rounded-3xl border border-yellow-200 bg-white p-4 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <h1 className="mt-1 text-xl font-black text-gray-950">Uniforms Received</h1>
          ...
          <>
            <section className="rounded-3xl border border-yellow-200 bg-white p-3 shadow-sm">
              <div className="grid gap-2">
                <input
                  ...
                  placeholder="Search student or uniform..."
            ...
            <section className="space-y-3">
              {loading ? (
                ...
                <div className="rounded-3xl bg-white p-5 text-center text-sm font-black text-gray-500 shadow-sm">
                  No uniforms received yet.
                </div>
              ) : (
                records.map((record) => (
                  <StudentUniformCard key={`${record.student_id}-${record.class_name}`} record={record} />
                ))
              )}
            </section>
          </>
...
function StudentUniformCard({ record }: { record: StudentUniformRecord }) {
    ...
    <article className="rounded-3xl border border-yellow-200 bg-white p-4 shadow-sm">
      ...
      <div className="mt-3 rounded-2xl bg-[#fffdf6] p-3">
        ...
        <div className="space-y-2">
          {record.uniforms.length === 0 ? (
            <div className="rounded-2xl bg-white px-3 py-2 text-sm font-black text-gray-400 shadow-sm">
              Payment made, but no uniform has been marked as given yet.
            </div>
          ) : (
            record.uniforms.map((uniform) => (
              <div
                key={uniform.key}
                className="flex items-center justify-between gap-3 rounded-2xl bg-white px-3 py-2 text-sm shadow-sm"
              >
                <span className="font-black text-gray-900">{uniform.item_name}</span>
                <span className="shrink-0 rounded-full bg-yellow-100 px-2 py-1 text-xs font-black text-yellow-800">
                  {uniform.quantity > 0 ? `x${uniform.quantity}` : "Not given"}
                </span>
              </div>
            ))
          )}
